...
gen confounder = rnormal(0,0.34777501)
...
gen exp_res = rnormal(0,0.34777501)
...
matrix U = (84.88533, 11.98352\ 11.98352, 4.608265)
...
gen UPDRS = 24 + u0 + (2.25 + u1)*disease_duration + (1/0.34777501)*confounder + (0.225/0.34777501)*confounder*disease_duration + (2.0/0.518)*exp + (0.45/0.518)*exp*disease_duration + lev1_res
...
postfile `memhold' beta_exp beta_int se_int beta_sl se_sl cov_int_sl using simulation_study_snp_ver4_confounders_results_dataset_`bob', replace
